...
#   
#   UPD<- ddply(subset(UnlumpedProjectionData,Policy!='Historic'),c('IdOrig','Policy'),mutate,NPV=cumsum(DiscProfits))
# 
# write.csv(file='wtf.csv',UPD)
# 
# Current<- subset(UnlumpedProjectionData,Year==2012 & Policy=='Historic')
# 
# Projection<- subset(UnlumpedProjectionData,Year==2050)
# 
# write.csv(file='Current.csv',Current)
# 
# write.csv(file='Projection.csv',Projection)
# 
# Final=ddply(UPD,c('Country','Policy'),summarize,NPV=sum(NPV[Year==2050],na.rm=T))