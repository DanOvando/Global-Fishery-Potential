...
DivyUpSofia <- function(ProjectionData, RawData)
{
  ...
  {
    sofiaRef$NumCntrys[a] <-
      length(unlist(str_split(sofiaRef$Country[a], pattern = ',')))
  }
  ...
  if (dim(sofiaRef)[1] > 0)
  {
    ...
    MissingSofiaCntry <-
      c('List of Sofia Stocks Where A Country Cannot be Found in FAO')
    ...
    for (n in 1:length(sofiaIds))
    {
      ...
      cntrys <-
        unlist(str_split(sofiaRef$Country[sofiaRef$IdOrig == sofiaIds[n]], pattern =
                           ', '))
      
      # Fao region (unlist in case there are multiple)
      regs <-
        unlist(str_split(sofiaRef$RegionFAO[sofiaRef$IdOrig == sofiaIds[n]], pattern =
                           ','))
      
      # Subset fao to sci name and regions that match multinational stock
      tempF <-
        f[f$SciName == sci &
            (f$Country %in% cntrys) & (f$RegionFAO %in% regs),
          c('IdOrig',
            'Country',
            'Year',
            'SciName',
      ...
      cntryPercs$PercOfTotal <-
        cntryPercs$TotalCatch / sum(cntryPercs$TotalCatch, na.rm = T)
      
      if (length(unique(cntryPercs$Country)) < length(cntrys))
      {
        MissingSofiaCntry <-
          append(MissingSofiaCntry,
                 sofiaIds[n],
                 after = length(MissingSofiaCntry))
      ...
      NewSofia <- as.data.frame(matrix(nrow = 0, ncol = ncol(tempSProj)))
      ...
      for (d in 1:length(cntrys))
      {
        ...
        unlumpSofia$Catch <-
          unlumpSofia$Catch * cntryPercs$PercOfTotal[cntryPercs$Country == cntrys[d]]
        
        unlumpSofia$Biomass <-
          unlumpSofia$Biomass * cntryPercs$PercOfTotal[cntryPercs$Country == cntrys[d]]
        
        unlumpSofia$MSY <-
          unlumpSofia$MSY * cntryPercs$PercOfTotal[cntryPercs$Country == cntrys[d]]
        
        unlumpSofia$k <-
          unlumpSofia$k * cntryPercs$PercOfTotal[cntryPercs$Country == cntrys[d]]
        
        unlumpSofia$Profits <-
          unlumpSofia$Profits * cntryPercs$PercOfTotal[cntryPercs$Country == cntrys[d]]
        
        unlumpSofia$DiscProfits <-
          unlumpSofia$DiscProfits * cntryPercs$PercOfTotal[cntryPercs$Country == cntrys[d]]
        NewSofia <- bind_rows(NewSofia, unlumpSofia)
    ...
    DivySofia <- bind_rows(DivySofia)
    
    # Combine results for country-level and multinational sofia stocks
    UnlumpSofia <- bind_rows(sofiaIndiv, DivySofia)
    
    Data <- Data[!(Data$Dbase == 'SOFIA'), ]
    
    FinalData <- bind_rows(Data, UnlumpSofia)
