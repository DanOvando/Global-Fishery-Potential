...
#   faoCatch<-ddply(fao[fao$Year==BaselineYear,],c('Country','Year'),summarize,TotalCatch=sum(Catch,na.rm=T))
  ...
  regs<- unique(eTest$RegionFAO) #c('37','51','27','21','77','34','81','48','47','31','87','57','41','71','67','61','88','58','18')
  ...
  OverallCostRevSummary<-as.data.frame(matrix(summary(eTest$CostRevRatio),nrow=1,ncol=length(summary(eTest$CostRevRatio))))
  ...
  SummaryTable<-as.data.frame(matrix(nrow=length(SpCats),ncol=9))
  colnames(SummaryTable)<-c('ISSCAAP Group','Stocks', 'Min','1st Quart','Median','Mean','3rd Quart','Max')
  
  SummaryTable2<-as.data.frame(matrix(nrow=length(SpCats),ncol=9))
