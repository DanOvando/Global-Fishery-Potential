...
{
  AllYearSummary <- Data %>%
    filter(Policy == 'Historic' | Policy == 'Catch Share Three') %>%
  ...
  YearSummary$Country <-
    reorder(YearSummary$Country, 1 / YearSummary$TotalBiomass)
  ...
  print(ggplot((YearSummary[YearSummary$BioRank, ][1:20, ]),
               aes(x = Country, y = TotalBiomass, fill = PercRam)
  ...
  YearSummary$Country <-
    reorder(YearSummary$Country, 1 / YearSummary$TotalCatch)
  
  
  pdf(paste(FigureFolder, '2012 Catch Rankings.pdf', sep = ''))
  print(ggplot((YearSummary[YearSummary$CatchRank, ][1:20, ]), aes(
    x = Country, y = TotalCatch, fill = PercRam
  )) + geom_bar(stat = 'identity')
  + theme(axis.text.x = element_text(
    angle = 45,
    hjust = 0.9,
  ...
  SpCat <- ProjectionData %>% 
    filter(Year == 2012) %>% 
    group_by(SpeciesCatName) %>% 
    summarize(MedianBvBmsy = median(BvBmsy, na.rm = T),
  ...
  ## Reorder fullname based on the the sum of the other columns
  SpCat$SpeciesCatName <-
    reorder(SpCat$SpeciesCatName, SpCat$MedianBvBmsy)
  
  pdf(paste(FigureFolder, '2012 BvBmsy by Species Category.pdf', sep = ''))
  print(
    ggplot(
      SpCat,
      aes(x = SpeciesCatName, y = MedianBvBmsy, fill = MeanRamBvBmsy)
    ) + geom_bar(stat = 'identity')
    + theme(axis.text.x = element_text(
      angle = 45,
      hjust = 0.9,
