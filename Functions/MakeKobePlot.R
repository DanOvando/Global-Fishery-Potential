...
KobeColors<- colorRampPalette(c("aliceblue", "white",'gold1'))
...
# par(mfrow=c(1,2))
layout(t(matrix(c(1,2))), widths = c(3.25,1), heights = c(1,1), respect = FALSE)
image(DensityData$x,DensityData$y,DensityData$z,col=KobeColors(50),xlab='B/Bmsy',ylab='F/Fmsy')
points(BaselineData$BvBmsy,BaselineData$FvFmsy,pch=16,col=(as.factor(BaselineData$Dbase)))
abline(h=1,v=1,lty=2)
plot.new()
par(mai=c(0,0,0,0))
legend('center',legend=unique(BaselineData$Dbase),pch=16,col=(as.factor(unique(BaselineData$Dbase))),cex=0.65,bty='n')
dev.off()
