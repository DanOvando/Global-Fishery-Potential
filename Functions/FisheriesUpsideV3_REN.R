...
FisheriesUpsideV3_REN<-function(Data,BaselineYear,DenominatorPolicy,RecoveryThreshold,LumpedName,SubsetName)
{
  ...
  NPV<- Data[Data$IdOrig %in% ids & Data$Policy!='Historic',] %>%
  ...
  CountryUpsides<- FisheryUpside %>%
    ...
    summarise(TotalCatch=sum(Catch,na.rm=T),TotalBiomass=sum(Biomass,na.rm=T),TotalProfits=sum(Profits,na.rm=T),
  ...
  GlobalBaseline<- Baseline %>%
    group_by(Policy) %>%
    summarize(Biomass_in_2012=sum(Biomass,na.rm=T),Profit_in_2012=sum(Profits,na.rm=T),Harvest_in_2012=sum(Catch,na.rm=T),
  ...
  GlobalResults<- FisheryUpside %>%
    group_by(Policy) %>%
    summarize(AnnuityOutTo2050=sum(FinalNPV,na.rm=T)*(0.05/(1-((1+0.05)^-38))),ProfitIn2050=sum(Profits,na.rm=T),BiomassIn2050=sum(Biomass,na.rm=T),HarvestIn2050=sum(Catch,na.rm=T))
  ...
  Denom_GlobalPercentage<- Data %>%
    group_by(CanProject) %>%
    summarize( Biomass_CatchShareVsToday_Rebuild=1,
