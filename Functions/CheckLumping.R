CheckLumping<- function(ProjectionData,UnlumpedProjectionData)
{
...
quartz()
ggplot(data=Summary,aes(x=Lumping,y=TotalCatch))+geom_bar(stat='identity',position='dodge')+facet_wrap(~Policy)

quartz()
ggplot(data=Summary,aes(x=Lumping,y=TotalMSY))+geom_bar(stat='identity',position='dodge')+facet_wrap(~Policy)

quartz()
ggplot(data=Summary,aes(x=Lumping,y=TotalProfits))+geom_bar(stat='identity',position='dodge')+facet_wrap(~Policy)

quartz()
ggplot(data=Summary,aes(x=Lumping,y=TotalBiomass))+geom_bar(stat='identity',position='dodge')+facet_wrap(~Policy)

quartz()
ggplot(data=Summary,aes(x=Lumping,y=MedianB))+geom_bar(stat='identity',position='dodge')+facet_wrap(~Policy)

quartz()
ggplot(data=Summary,aes(x=Lumping,y=MedianF))+geom_bar(stat='identity',position='dodge')+facet_wrap(~Policy)
...
pdf(file='Lump Catch Change.pdf',width=12,height=6)
  (ggplot(data=CountryDiff,aes(x=Country,y=LumpChange))+geom_bar(stat='identity',position='dodge')+
  theme(axis.text.x=element_text(angle=45,hjust=0.9,vjust=0.9,size=8)))
dev.off()
