...
SnowNEIs2 <-
  ...
  {
    write.table((paste(
      round(100 * (s / length(nei_stock)), 2), '% done with SnowNEIs2', sep =
        ''
    )),
    file = 'SnowNEIs Progress.txt',
    append = TRUE,
    sep = ";",
    dec = ".",
    row.names = FALSE,
    ...
    species <-
      SpeciesLevel[SpeciesLevel$SpeciesCatName %in% neis$SpeciesCatName, ]
    
    # Find row with taxon information in species AFSIS
    WhereNEI <- match(NeiCat, Spec_ISSCAAP$Species_AFSIS)
    
    JStocks <-
      NA# fill this vector with the number of Jstocks for each nei group
    VarBvBmsy <-
      NA # fill this vector with the variance in BvBmsy for each nei group
    VarFvFmsy <-
      NA # fill this vector with the variance in FvFmsy for each nei group
    Year <- NA
    
    if (unique(stats$TaxonLevel) == "Genus")
      # find scientific names for all genus level nei stocks
    {
      Genus <- unlist(str_split(NeiCat, pattern = " "))[1] # pull out genus
      
      WhereComp <-
        grepl(Genus, Spec_ISSCAAP$Species_AFSIS) # search for species names in that genus
      
      compstocks <-
        unique(Spec_ISSCAAP$Species_AFSIS[WhereComp]) # pull out species names
      
    }
    
    if (unique(stats$TaxonLevel) == "Non-Genus" &
        (NeiCat %in% Spec_ISSCAAP$Family))
      # determine if non-genus stock is a family name
    {
    ...
    if (unique(stats$TaxonLevel) == "Non-Genus" &
        (tolower(NeiCat) %in% tolower(Spec_ISSCAAP$Order)))
      # determine if non-genus stock is an order name
    {
      order <-
        toupper(NeiCat) # order of nei stock (translate to uppercase to match sheet)
    ...
    {
      ComparisonStocks <-
        SpeciesLevel[SpeciesLevel$SciName %in% compstocks, ] # pulling comparable stocks from the globe
      ...
      if (nrow(ComparisonStocks) > 0)
      {
        ...
        results <- ComparisonStocks %>%
          ...
          summarize(
            ...
            VarBvBmsy = var(BvBmsy, na.rm = T),
            VarFvFmsy = var(FvFmsy, na.rm = T)
          )
        ...
        if (length(unique(results$Policy)) > 1)
        {
          for (b in 1:nrow(results))
          {
            ...
            WhereNei <-
              neis$SciName == NeiCat &
              neis$Year == results$Year[b] &
              neis$Policy == results$Policy[b]
            neis[WhereNei, VarsToFill] <-
              results[b, c("BvBmsy25",
                           "FvFmsy75",
                           "MedianG",
                           "MedianK",
                           "MedianPrice",
