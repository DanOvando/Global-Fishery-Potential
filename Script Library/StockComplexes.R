StockComplexes<- function(Data,OverlapMode,stringsAsFactors=F)
{
  ...
  #     FilterData<- RawData
    ...
    ramSubset<-subset(FilterData,Dbase=="RAM")
    sofiaSubset<-subset(FilterData,Dbase=="SOFIA")
    faoSubset<-subset(FilterData,Dbase=="FAO")
    ...
    RamNationalComplexes<-unique(newRam[c("Country","SciName","IdOrig","SOverlapId","FOverlapId")])
    RamNationalComplexes<-RamNationalComplexes[is.na(RamNationalComplexes$SOverlapId)==F | is.na(RamNationalComplexes$FOverlapId)==F,]
    ...
    for (i in 1:nrow(multinational)){
      ...
      if(length(overlapstocks)>0){
        
        RamMultiNatComplex<- c(multinational$SciName[i],multinational$Country[i],multinational$IdOrig[i],overlapstocks)
     
          if(i==1)
          {
            length(RamMultiNatComplex)<-500
            RamMultiNatComplexFinal<-RamMultiNatComplex
          }
          
          if(m>1)
          {
            length(RamMultiNatComplex)<-500
            RamMultiNatComplexFinal<-rbind(RamMultiNatComplexFinal,RamMultiNatComplex)
