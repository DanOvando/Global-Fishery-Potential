...
FindCatchShares<-function(DataR,)
...
GFRstocks<-unique(RawData[,c('IdOrig','Dbase','Country','CommName','SciName','RegionFAO','CatchShare')])
...
fao2<-subset(RawData,IdOrig %in% faoCSids & Year==2012 & is.na(Catch)==F)
...
matchM<-join(multiRam,fao,by=c('RegionFAO','SciName'),type='inner')
...
Countries<-c('Argentina','Canada','Chile','China','Denmark','Iceland','India','Indonesia','Japan','Malaysia','Mexico','Morocco','Namibia','Norway','Peru','Philippines',
             'Republic of Korea','Russian Federation','Senegal','South Africa','Spain','Thailand','United Kingdom','USA','Viet Nam')

CSdata<-subset(RawData,Dbase=='FAO' & Country %in% Countries)

CSdata$CatchShare<-NA

CSdata$CatchShare[CSdata$IdOrig %in% matchF$IdOrig]<-'Catch Shares'

CSdata$CatchShare[!(CSdata$IdOrig %in% matchF$IdOrig)]<-'Other'

CSpercs<-ddply(CSdata[CSdata$Year==2012,],c('Country','CatchShare'),summarize,TotalCatch=sum(Catch,na.rm=T))

CSpercs<-ddply(CSpercs,c('Country'),mutate,AllCatch=sum(TotalCatch,na.rm=T),Perc=100*(TotalCatch/AllCatch))

pdf(file='Catch Share Fractions.pdf',width=8,height=11)

ggplot(CSpercs,aes(x=Country,y=Perc,fill=as.factor(CatchShare))) +
