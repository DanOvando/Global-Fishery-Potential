...
DatabaseBuild<- function()
{
  ...
  ColNames = c("Id","IdOrig","Dbase", "Year","Catch","CatchUnit", "Fmort","FmortUnit","FmortMetric", "SciName","CommName",'TaxCode',
  ...
  FAO=read.csv("Data/FAO_1950to2012_111914.csv",header=T,stringsAsFactors=F,na.strings=c("...","-","0 0")) # convert ... and - to NA in catch record
  FAO[,9:71]=apply(FAO[,9:71],2,function(y) as.numeric(gsub(" F","",y))) # remove " F" from certain data points and convert catch record to numeric
  ...
  names(FAO)[5]="TaxCode"
  names(FAO)[6]="SciName"
  names(FAO)[7]="RegionFAO"
  names(FAO)[8]<-'CatchUnit'
  ...
  fao=reshape(FAO,varying=9:71,direction="long", v.names="Catch", timevar="Year",times=1950:2012,)
  ...
  ####### Species, Country, Region, and Taxon Codes ####### 
  ...
  # Add in TaxCode by matching scientific names to FAO
  
  species<-unique(RAM$SciName)
  
  for (a in 1:length(species)) # match group code to group name
  {
    match<-match(species[a],fao$SciName)
    
    if(is.na(match)==F)
    {
      where<-RAM$SciName==species[a]
      
      RAM$TaxCode[where]<-fao$TaxCode[match]
    }
  }
  ...
  # Add in TaxCodes using fao TaxCode
  species<-unique(sofia$SciName)
  
  for (b in 1:length(species)) # match group code to group name
  {
    match<-match(species[b],fao$SciName)
    
    if(is.na(match)==F)
    {
      where<-sofia$SciName==species[b]
      
      sofia$TaxCode[where]<-fao$TaxCode[match]
    }
  }
