...
#   dev.off()
  ...
  FullProjectionData<- RunProjection(MsyData[MsyData$CanProject==T,],BaselineYear,NumCPUs,StatusQuoPolicy,Policies = Policies,
                                     Discount = Discount) #Run projections on MSY data that can be projected
...
ProjectionData <- BuildPolicyBAUs(ProjectionData,BaselineYear,elastic_demand = elastic_demand, elasticity = elasticity,
                                  Discount = Discount,sp_group_demand = sp_group_demand,beta = beta)
